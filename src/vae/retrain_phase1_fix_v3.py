...
def _snapshot(seed: int, device: str) -> dict:
    ...
    snap.update({"phase": "phaseBCD_v3", "target_classes": list(TARGET_CLASSES)})
    return snap
...
def _config_for(class_name: str) -> dict:
    ...
    if class_name == "BruteForce":
        cfg["latent_dim"]["BruteForce"] = 4
        cfg["weight_decay"] = 1e-4
        cfg["checkpoint_name_override"] = "vae_bruteforce_v3.pt"
        cfg["curves_name_override"] = "curves_BruteForce_v3.png"
        ...
        cfg["checkpoint_name_override"] = "vae_dos_v3.pt"
        cfg["curves_name_override"] = "curves_DoS_v3.png"
        cfg["continuous_feature_loss_weights"] = {
            "Variance": 20.0,
            "Tot sum": 10.0,
            "Rate": 6.0,
...
def _checkpoint_path(class_name: str) -> Path:
    filename = "vae_bruteforce_v3.pt" if class_name == "BruteForce" else "vae_dos_v3.pt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tail-focused VAE retraining for BruteForce and DoS.")
    ...
    for class_name in TARGET_CLASSES:
        ...
        ckpt_v3 = _checkpoint_path(class_name)
        ...
        phase_d[class_name] = {
            ...
            "v3_reconstruction": _recon_summary(router, class_id, x_test, ckpt_v3),
            "mahalanobis_v3": _mahalanobis_summary(
                router,
                class_id,
                val_split["X"],
                val_split["y_8"],
            ...
            "pipeline_trace_v3": _pipeline_trace(router, class_id, x_test, ckpt_v3),
        }
    ...
    out_path = run_logger.run_dir / "phaseBCD_v3_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print("=== Phase B/C/D v3 Results ===")
    ...
    for class_name in TARGET_CLASSES:
        ...
        print(
            ...
            f"v3 median/p95={row['v3_reconstruction']['median']:.4f}/{row['v3_reconstruction']['p95']:.4f} "
            f"MD self-rate={row['mahalanobis_v3']['outlier_rate'] * 100.0:.2f}%"
        )
        print(
            f"  trace v2={row['pipeline_trace_v2']['pred_original']}->{row['pipeline_trace_v2']['pred_reconstructed']} "
            f"v3={row['pipeline_trace_v3']['pred_original']}->{row['pipeline_trace_v3']['pred_reconstructed']}"
        )

...
def _snapshot(seed: int, device: str) -> dict:
    ...
    snap.update({"phase": "phaseBCD_v4", "target_classes": list(TARGET_CLASSES)})
    return snap
...
def _config_for(class_name: str) -> dict:
    ...
    if class_name == "BruteForce":
        ...
        cfg["checkpoint_name_override"] = "vae_bruteforce_v4.pt"
        cfg["curves_name_override"] = "curves_BruteForce_v4.png"
        cfg["raw_relative_continuous_loss_weight"] = 3.0
        ...
        cfg["checkpoint_name_override"] = "vae_dos_v4.pt"
        cfg["curves_name_override"] = "curves_DoS_v4.png"
        cfg["raw_relative_continuous_loss_weight"] = 5.0
        cfg["continuous_feature_loss_weights"] = {
            "Variance": 20.0,
            "Tot sum": 12.0,
            "Max": 5.0,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tail-aware raw-relative VAE retraining for BruteForce and DoS.")
    ...
    for class_name in TARGET_CLASSES:
        ...
        ckpt_v4 = _checkpoint_path(class_name, "v4")
        ...
        phase_d[class_name] = {
            ...
            "v4_reconstruction": _recon_summary(router, class_id, x_test, ckpt_v4),
            "mahalanobis_v4": _mahalanobis_summary(
                router,
                class_id,
                val_split["X"],
                val_split["y_8"],
            ...
            "pipeline_trace_v4": _pipeline_trace(router, class_id, x_test, ckpt_v4),
        }
    ...
    out_path = run_logger.run_dir / "phaseBCD_v4_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print("=== Phase B/C/D v4 Results ===")
    ...
    for class_name in TARGET_CLASSES:
        ...
        print(
            ...
            f"v4 median/p95={row['v4_reconstruction']['median']:.4f}/{row['v4_reconstruction']['p95']:.4f} "
            f"MD self-rate={row['mahalanobis_v4']['outlier_rate'] * 100.0:.2f}%"
        )
        print(
            f"  trace v2={row['pipeline_trace_v2']['pred_original']}->{row['pipeline_trace_v2']['pred_reconstructed']} "
            f"v4={row['pipeline_trace_v4']['pred_original']}->{row['pipeline_trace_v4']['pred_reconstructed']}"
        )

...
def _snapshot(seed: int, device: str) -> dict:
    ...
    snap.update({"phase": "phaseBCD", "target_classes": list(TARGET_CLASSES)})
    return snap
...
def _config_for(class_name: str) -> dict:
    ...
    if class_name == "BruteForce":
        ...
        cfg["checkpoint_name_override"] = "vae_bruteforce_v2.pt"
        cfg["curves_name_override"] = "curves_BruteForce_v2.png"
    elif class_name == "DoS":
        cfg["latent_dim"]["DoS"] = 16
        cfg["checkpoint_name_override"] = "vae_dos_v2.pt"
        cfg["curves_name_override"] = "curves_DoS_v2.png"
    else:
        raise ValueError(class_name)
    return cfg


def _checkpoint_path(class_name: str) -> Path:
    filename = "vae_bruteforce_v2.pt" if class_name == "BruteForce" else "vae_dos_v2.pt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Retrain BruteForce and DoS VAE checkpoints for Phase 1.")
    ...
    for class_name in TARGET_CLASSES:
        class_id = CLASS_TO_ID[class_name]
        ckpt_v2 = _checkpoint_path(class_name)
        idx = np.where(test_split["y_8"] == class_id)[0][:1000]
        x_test = test_split["X"][idx].astype(np.float32)
        phase_d[class_name] = {
            "old_reconstruction": _recon_summary(router, class_id, x_test, None),
            "new_reconstruction": _recon_summary(router, class_id, x_test, ckpt_v2),
            "mahalanobis_new": _mahalanobis_summary(
                router,
                class_id,
                val_split["X"],
                val_split["y_8"],
            ...
            "pipeline_trace_old": _pipeline_trace(router, class_id, x_test, None),
            "pipeline_trace_new": _pipeline_trace(router, class_id, x_test, ckpt_v2),
        }
    ...
    out_path = run_logger.run_dir / "phaseBCD_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print("=== Phase B/C/D Results ===")
    ...
    for class_name in TARGET_CLASSES:
        ...
        print(
            f"{class_name:10s} old median/p95={row['old_reconstruction']['median']:.4f}/{row['old_reconstruction']['p95']:.4f} "
            f"new median/p95={row['new_reconstruction']['median']:.4f}/{row['new_reconstruction']['p95']:.4f} "
            f"MD self-rate={row['mahalanobis_new']['outlier_rate'] * 100.0:.2f}%"
        )
        print(
            f"  trace old={row['pipeline_trace_old']['pred_original']}->{row['pipeline_trace_old']['pred_reconstructed']} "
            f"new={row['pipeline_trace_new']['pred_original']}->{row['pipeline_trace_new']['pred_reconstructed']}"
        )

...
def _snapshot(seed: int, device: str) -> dict:
    ...
    snap.update({"phase": "phaseBCD_v6", "target_classes": list(TARGET_CLASSES)})
    return snap
...
def _config_for(class_name: str) -> dict:
    ...
    if class_name == "BruteForce":
        ...
        cfg["beta_target"]["BruteForce"] = 0.5
        cfg["checkpoint_name_override"] = "vae_bruteforce_v6.pt"
        cfg["curves_name_override"] = "curves_BruteForce_v6.png"
        ...
        cfg["train_sample_weight_rules"] = [
            {"feature": "Rate", "mode": "high_quantile", "quantile": 0.90, "bonus": 3.0},
            {"feature": "ack_count", "mode": "high_quantile", "quantile": 0.80, "bonus": 1.5},
            {"feature": "Header_Length", "mode": "high_quantile", "quantile": 0.80, "bonus": 1.0},
            {"feature": "IAT", "mode": "low_quantile", "quantile": 0.20, "bonus": 1.0},
            {"feature": "TCP", "mode": "binary_on", "bonus": 1.0},
            {"feature": "SSH", "mode": "binary_on", "bonus": 0.5},
        ]
    elif class_name == "DoS":
        cfg["latent_dim"]["DoS"] = 16
        cfg["beta_target"]["DoS"] = 1.0
        cfg["checkpoint_name_override"] = "vae_dos_v6.pt"
        cfg["curves_name_override"] = "curves_DoS_v6.png"
        cfg["continuous_feature_loss_weights"] = {
            "Variance": 20.0,
            "Tot sum": 10.0,
            "Rate": 6.0,
        ...
        cfg["train_sample_weight_rules"] = [
            {"feature": "Variance", "mode": "high_quantile", "quantile": 0.90, "bonus": 3.0},
            {"feature": "Tot sum", "mode": "high_quantile", "quantile": 0.90, "bonus": 2.5},
            {"feature": "Max", "mode": "high_quantile", "quantile": 0.85, "bonus": 1.5},
            {"feature": "TCP", "mode": "binary_on", "bonus": 0.75},
        ]
    else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Weighted-sampler VAE retraining for BruteForce and DoS.")
    ...
    for class_name in TARGET_CLASSES:
        ...
        ckpt_v6 = _checkpoint_path(class_name, "v6")
        ...
        phase_d[class_name] = {
            ...
            "v6_reconstruction": _recon_summary(router, class_id, x_test, ckpt_v6),
            "mahalanobis_v6": _mahalanobis_summary(
                router,
                class_id,
                val_split["X"],
                val_split["y_8"],
            ...
            "pipeline_trace_v6": _pipeline_trace(router, class_id, x_test, ckpt_v6),
        }
    ...
    out_path = run_logger.run_dir / "phaseBCD_v6_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print("=== Phase B/C/D v6 Results ===")
    ...
    for class_name in TARGET_CLASSES:
        ...
        print(
            ...
            f"v6 median/p95={row['v6_reconstruction']['median']:.4f}/{row['v6_reconstruction']['p95']:.4f} "
            f"MD self-rate={row['mahalanobis_v6']['outlier_rate'] * 100.0:.2f}%"
        )
        print(
            f"  trace v2={row['pipeline_trace_v2']['pred_original']}->{row['pipeline_trace_v2']['pred_reconstructed']} "
            f"v6={row['pipeline_trace_v6']['pred_original']}->{row['pipeline_trace_v6']['pred_reconstructed']}"
        )
